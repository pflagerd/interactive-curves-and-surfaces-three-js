...
//
// $y = scale(x-vertex_x)^2 + vertex_y$
//
class ParabolaInVertexFormBufferGeometry extends BufferGeometry {
    vertices = [];

    evaluate(x) {
        return this.scale * (x - this.vertex.x) * (x - this.vertex.x) + this.vertex.y;
    }
...
function main() {
    ...
    const parabolaInVertexFormBufferGeometry = new ParabolaInVertexFormBufferGeometry(0, 3.6, -0.15);
    const parabolaInVertexFormLine = new THREE.Line(parabolaInVertexFormBufferGeometry, material);
    scene.add( parabolaInVertexFormLine );
